...
class ListViewItem extends Component {
    ...
    render() {
        ...
        return ( 
            <TouchableHighlight 
            underlayColor = {'#eee'} 
            style = {{paddingTop: 6, paddingBottom: 6, backgroundColor: "#F8F8F8", borderColor: '#eee'}} { ... this.props.sortHandlers}>
                <View style = {{flexDirection: 'row',alignItems: 'center'}}>
                    <CheckBox data = {data} color = { color } onCheckBoxPressed = { this.onCheckBoxPressed }></CheckBox>
                    <Text style = {{fontSize:18, color: color, textDecorationLien: textDecorationLine}}>{data.title}</Text>
                </View>
            </TouchableHighlight>
        )
    }
